...
static struct class *rockpi_mcu_class;
...
static ssize_t rockpi_mcu_brightness_show(struct class *cls,
...
static ssize_t rockpi_mcu_brightness_store(struct class *cls,
			struct class_attribute *attr,
			const char *buf, size_t count)
{
	...
	ret = kstrtoint(buf, 10, &bright);
	if (ret) {
		LOG_ERR("kstrtoint failed, ret = %d\n", ret);
		return ret;
	}
	...
	rockpi_mcu_set_bright(bright);

	brightness = bright;

	LOG_INFO("brightness = 0x%x\n", brightness);
...
static CLASS_ATTR(connection, 0444, rockpi_mcu_connection_show, NULL);
			...
			const struct i2c_device_id *id)
{
	...
	rockpi_mcu_class = class_create(THIS_MODULE, "rockpi_mcu");

	ret =  class_create_file(rockpi_mcu_class, &class_attr_connection);
	if (ret)
		LOG_ERR("Fail to create class attributes connection.\n");

	ret =  class_create_file(rockpi_mcu_class, &class_attr_brightness);
	if (ret)
		LOG_ERR("Fail to create class attributes brightness.\n");
